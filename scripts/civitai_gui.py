...
def show_multi_buttons(model_list, type_list, version_value):
    ...
    return (gr.Button.update(visible=multi, interactive=multi), # Download Multi Button
            ...
            gr.HTML.update(visible=multi, interactive=multi_file_subfolder, value=_api.create_button_group_html("Sub folder for selected files:", sub_folders, default_subfolder, "subfolder_selected_buttons", group_class="button-group-chips")) # Selected type sub folder
            )
...
def on_ui_tabs():    
    ...
    with gr.Blocks() as civitai_interface:
        with gr.Tab(label="Browser", elem_id="browserTab"):
            ...
            with gr.Row():
                with gr.Column(scale=4):
                    # gr.Dropdown(label="Model:", choices=[], interactive=False, elem_id="quicksettings1", value=None)
                    list_models_html = gr.HTML(value=_api.create_button_group_html("Model:", [], None, "model_selection_buttons"))
                    # gr.Dropdown(label="Version:", choices=[], interactive=False, elem_id="quicksettings0", value=None)
                    list_versions_html = gr.HTML(value=_api.create_button_group_html("Version:", [], None, "version_selection_buttons", group_class="button-group-chips"))
                with gr.Column(scale=2):
                    # gr.Dropdown(label="File:", choices=[], interactive=False, elem_id="file_list", value=None)
                    file_list_html = gr.HTML(value=_api.create_button_group_html("File:", [], None, "file_selection_buttons"))
            with gr.Row():
                with gr.Column(scale=4):
                    install_path = gr.Textbox(label="Download folder:", interactive=False, max_lines=1)
                with gr.Column(scale=2):
                    # gr.Dropdown(label="Sub folder:", choices=[], interactive=False, value=None)
                    sub_folder_html = gr.HTML(value=_api.create_button_group_html("Sub folder:", [], None, "subfolder_selection_buttons"))
            with gr.Row():
                with gr.Column(scale=4):
        ...
        model_select = gr.Textbox(elem_id="model_select", visible=False) # This is used by javascript select_model for card click
        ...
        # New hidden textboxes for button selections
        model_button_select = gr.Textbox(elem_id="model_button_select", visible=False)
        version_button_select = gr.Textbox(elem_id="version_button_select", visible=False)
        file_button_select = gr.Textbox(elem_id="file_button_select", visible=False)
        subfolder_button_select = gr.Textbox(elem_id="subfolder_button_select", visible=False)
        ...
        def update_models_dropdown(input_model_string):
            if not gl.json_data:
                return (
                    gr.HTML.update(value=_api.create_button_group_html("Model:", [], None, "model_selection_buttons")), # List models
                    gr.HTML.update(value=_api.create_button_group_html("Version:", [], None, "version_selection_buttons", group_class="button-group-chips")), # List version
                    ...
                    gr.HTML.update(value=_api.create_button_group_html("Sub folder:", [], None, "subfolder_selection_buttons")), # Sub folder
                    ...
                    gr.HTML.update(value=_api.create_button_group_html("File:", [], None, "file_selection_buttons")), # File list
            ...
            model_string = re.sub(r'\.\d{3}$', '', input_model_string)
            model_name, model_id = _api.extract_model_info(model_string)
            # _api.update_model_versions will return (choices_list, default_value)
            model_versions_data = _api.update_model_versions(model_id) # (choices_list, default_value)
            
            # Use the default value from model_versions_data for update_model_info
            (html, tags, base_mdl, DwnButton, SaveImages, DelButton, filelist_choices, filename, dl_url, id, current_sha256, install_path, sub_folder_choices) = _api.update_model_info(model_string, model_versions_data[1])
            
            return (
                    gr.HTML.update(value=_api.create_button_group_html("Model:", [model_string], model_string, "model_selection_buttons")),
                    gr.HTML.update(value=_api.create_button_group_html("Version:", model_versions_data[0], model_versions_data[1], "version_selection_buttons", group_class="button-group-chips")),
                    html,tags,base_mdl,filename,install_path,
                    gr.HTML.update(value=_api.create_button_group_html("Sub folder:", sub_folder_choices, None, "subfolder_selection_buttons")), # Sub folder
                    DwnButton,SaveImages,DelButton,
                    gr.HTML.update(value=_api.create_button_group_html("File:", filelist_choices, None, "file_selection_buttons")), # File list
                    dl_url,id,current_sha256,
        ...
        version_button_select.change(
            fn=_api.update_model_info,
            inputs=[
                model_button_select, # Model selection now comes from this textbox
                version_button_select # Version selection now comes from this textbox
                ],
            outputs=[
                preview_html_input,
                trained_tags,
                ...
                save_images,
                delete_model,
                file_list_html, # Now HTML
                model_filename,
                dl_url,
                model_id,
                current_sha256,
                install_path,
                sub_folder_html # Now HTML
            ]
        )
        
        file_button_select.change(
            fn=_api.update_file_info,
            inputs=[
                model_button_select, # Model selection now comes from this textbox
                version_button_select, # Version selection now comes from this textbox
                file_button_select # File selection now comes from this textbox
            ],
            outputs=[
                model_filename,
                dl_url,
        ...
        download_finish.change(
            ...
            inputs=[
                model_filename,
                version_button_select, # Use version_button_select as input
                model_id
                ],
            outputs=[
                download_model,
                ...
                delete_model,
                download_progress,
                list_versions_html # Changed from list_versions to list_versions_html
            ]
        )
        ...
        delete_model.click(
            ...
            inputs=[
                delete_finish,
                model_filename,
                model_button_select, # Use model_button_select as input
                version_button_select, # Use version_button_select as input
                current_sha256,
                selected_model_list
                ],
        ...
        save_info.click(
            ...
            inputs=[
                install_path,
                model_filename,
                subfolder_button_select, # Use subfolder_button_select as input
                current_sha256,
                preview_html_input
                ],
            outputs=[]
        ...
        page_outputs = [
            list_models_html,      # Changed from list_models
            list_versions_html,    # Changed from list_versions
            list_html_input,
            get_prev_page,
            get_next_page,
            page_slider,
            ...
            delete_model,
            install_path,
            sub_folder_html,       # Changed from sub_folder
            file_list_html,        # Changed from file_list
            preview_html_input,
            trained_tags,
            base_model,
            model_filename
