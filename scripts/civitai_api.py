...
def update_model_versions(model_id, json_input=None):
    ...
    for item in api_json['items']:
        if int(item['id']) == int(model_id):
            ...
            return (display_version_names, default_value)
    
    return ([], None) # Return empty list and None for default when no model is found.
...
def create_button_group_html(label, choices, current_value, html_id, group_class="button-group"):
    html = f"<div class='filter-header'>{label}</div>"
    html += f'<div class="{group_class}" id="{html_id}">'
    for choice in choices:
        display_choice = escape(str(choice))
        is_selected = " selected" if str(choice) == str(current_value) else ""
        # The JavaScript function selectButtonGroupOption will be defined in civitai-html.js
        html += f'<label class="button-group-label{is_selected}" onclick="selectButtonGroupOption(\'{html_id}\', \'{display_choice}\')">{display_choice}</label>'
    html += '</div>'
    return html
