...
function updateCardSize(width, height) {
    ...
    // New logic for scaling the model-info-pill
    const originalPillFontSize = 0.75; // in em
    const originalPillPaddingVertical = 3; // in px
    const originalPillPaddingHorizontal = 8; // in px
    const originalPillGap = 5; // in px
    const originalPillTopLeft = 5; // in px

    const newPillFontSize = originalPillFontSize * scaleFactor;
    const newPillPaddingVertical = originalPillPaddingVertical * scaleFactor;
    const newPillPaddingHorizontal = originalPillPaddingHorizontal * scaleFactor;
    const newPillGap = originalPillGap * scaleFactor;
    const newPillTopLeft = originalPillTopLeft * scaleFactor;

    var pillKeyframes = `
        font-size: ${newPillFontSize}em !important;
        padding: ${newPillPaddingVertical}px ${newPillPaddingHorizontal}px !important;
        gap: ${newPillGap}px !important;
        top: ${newPillTopLeft}px !important;
        left: ${newPillTopLeft}px !important;
    `;
    addOrUpdateRule(styleSheet, '.civmodelcard .model-info-pill', pillKeyframes);
...
// Function to handle clicks on the new button groups
function selectButtonGroupOption(html_id, value) {
    const container = document.getElementById(html_id);
    if (!container) return;

    // Remove 'selected' class from all buttons in this group
    const buttons = container.querySelectorAll('.button-group-label');
    buttons.forEach(button => button.classList.remove('selected'));

    // Add 'selected' class to the clicked button
    // The event target (this) might be the label itself or a child element
    const clickedButton = event.currentTarget;
    if (clickedButton) {
        clickedButton.classList.add('selected');
    }

    // Find the corresponding hidden textbox and update its value
    let hiddenTextboxId;
    switch (html_id) {
        case 'model_selection_buttons':
            hiddenTextboxId = '#model_button_select textarea';
            break;
        case 'version_selection_buttons':
            hiddenTextboxId = '#version_button_select textarea';
            break;
        case 'file_selection_buttons':
            hiddenTextboxId = '#file_button_select textarea';
            break;
        case 'subfolder_selection_buttons':
            hiddenTextboxId = '#subfolder_button_select textarea';
            break;
        case 'subfolder_selected_buttons':
            hiddenTextboxId = '#subfolder_selected textarea'; // for multi-download subfolder
            break;
        default:
            console.error('Unknown button group ID:', html_id);
            return;
    }

    const hiddenTextbox = gradioApp().querySelector(hiddenTextboxId);
    if (hiddenTextbox) {
        hiddenTextbox.value = value;
        updateInput(hiddenTextbox);
    }
}
